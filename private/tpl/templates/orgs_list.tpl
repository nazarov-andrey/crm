...
<table class="tbl">
	...
	{foreach $orgs as $i => $org}
		<tr>
			...
			<td class="acts">
				<a href="/?action=persons_list&org={$org->getId()}">persons</a><br/>
				<a href="/?action=edit_org&id={$org->getId()}">modify</a><br />
				{if $smarty.session.super}<a href="/?action=remove_org&id={$org->getId()}">delete</a>{/if}
			</td>
		</tr>
	{/foreach}
</table>
